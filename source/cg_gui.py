...
class MyCanvas(QGraphicsView):
    ...
    def mouseReleaseEvent(self, event: QMouseEvent) -> None:
        ...
        self.main_window.updateUI(self.status, self.status, self.temp_algorithm)
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.clip_sutherland_hodgeman_btn   = QPushButton(QIcon('asset/icon/clip_sutherland_hodgeman.svg'), '')    
        ...
        self.clip_sutherland_hodgeman_btn   .setToolTip('Sutherland-Hodgeman算法裁剪多边形')
        ...
        vbox_layout1.addWidget(self.clip_sutherland_hodgeman_btn )
        ...
        self.clip_sutherland_hodgeman_btn   .clicked.connect(self.clip_sutherland_hodgeman_action   )
    ...
    def clip_sutherland_hodgeman_action(self):
        self.canvas_widget.start_clip('clip','Sutherland-Hodgeman')
    ...
    def updateUI(self, old = '', new = '', algorithm = ''):
        ...
        self.clip_sutherland_hodgeman_btn   .setChecked(False)
        if self.canvas_widget.temp_last_point != 0:
            self.mouse_btn                      .setDisabled(True)
            self.line_naive_btn                 .setDisabled(True)
            self.line_dda_btn                   .setDisabled(True)
            self.line_bresenham_btn             .setDisabled(True)
            self.polygon_dda_btn                .setDisabled(True)
            self.polygon_bresenham_btn          .setDisabled(True)
            self.ellipse_btn					.setDisabled(True)
            self.curve_bezier_btn				.setDisabled(True)
            self.curve_b_spline_btn				.setDisabled(True)
            ...
            self.clip_sutherland_hodgeman_btn   .setDisabled(True)
        ...
        if self.canvas_widget.selected_id == '':
            ...
            if self.canvas_widget.item_dict[self.canvas_widget.selected_id].item_type == 'line':
                ...
                self.clip_cohen_sutherland_btn  .setDisabled(True)
                self.clip_liang_barsky_btn      .setDisabled(True)
            if self.canvas_widget.item_dict[self.canvas_widget.selected_id].item_type == 'polygon':
                self.clip_sutherland_hodgeman_btn.setDisabled(False)
            else:
                self.clip_sutherland_hodgeman_btn.setDisabled(True)
